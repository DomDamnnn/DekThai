            ...
            <div className="flex justify-between items-start mb-1">
              ...
              {task.isGroup && (
                <Badge variant="outline" className="flex items-center gap-1 border-blue-200 text-blue-600 bg-blue-50">
                  ...
                  <span className="text-[10px]">งานกลุ่ม</span>
                </Badge>
              )}
            </div>
        ...
        <div className="flex justify-between items-start gap-3">
          <div className="space-y-1">
            ...
            <span className="text-[10px] text-muted-foreground flex items-center gap-1">
              <Clock className="w-3 h-3" /> {formatDateThai(notification.timestamp)}
            </span>
          </div>
    ...
    <Card className="border-none relative overflow-hidden text-white shadow-xl shadow-primary/20">
      ...
      <CardHeader className="relative p-5 pb-2">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-medium uppercase tracking-widest text-white/80">Smart Priority</span>
        </div>
        <CardTitle className="text-lg leading-tight">{task.title}</CardTitle>
        <p className="text-sm text-white/80">วิชา: {task.subject}</p>
      </CardHeader>
      ...
      <CardContent className="relative p-5 pt-0 space-y-4">
        <div className="bg-white/10 backdrop-blur-md rounded-xl p-3 border border-white/10">
          <div className="flex items-start gap-2">
            ...
            <p className="text-xs leading-relaxed text-white/90">
              <span className="font-bold">เหตุผล AI:</span> {task.priorityReason}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between text-xs">
          ...
          <div className="flex items-center gap-1 font-bold">
            ...
            <span>~{task.estimatedMinutes} นาที</span>
          </div>
        </div>
      </CardContent>
