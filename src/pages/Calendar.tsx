...
const formatDateHeader = (value: Date | undefined, language: "th" | "en") => {
  if (!value) return language === "th" ? "เลือกวันที่" : "Select a date";
  return value.toLocaleDateString(language === "th" ? "th-TH" : "en-US", {
    weekday: "long",
    day: "numeric",
    month: "long",
    year: "numeric",
...
const formatTime = (value: string, language: "th" | "en") => {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return "-";
  return date.toLocaleTimeString(language === "th" ? "th-TH" : "en-US", { hour: "2-digit", minute: "2-digit" });
};
    ...
    <Layout>
      <div className="flex flex-col gap-5 pb-24">
        <section className="pt-4 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">{isTeacher ? tx("ปฏิทินครู", "Teacher Calendar") : tx("ปฏิทินของฉัน", "My Calendar")}</h1>
            <p className="text-sm text-muted-foreground">
              {isTeacher
                ? tx("แตะวันที่เพื่อดูงานที่ประกาศและกำหนดส่ง", "Tap a day to see assignments posted and deadlines.")
                : tx("แตะวันที่เพื่อดูงานที่ได้รับมอบหมายและกำหนดส่ง", "Tap a day to see assigned work and deadlines.")}
            </p>
          </div>
          ...
          <Card className="border-none shadow-sm overflow-hidden rounded-[24px]">
            <CardContent className="p-4 flex flex-col items-center gap-3">
              <CalendarUI
                ...
                className="w-full"
                locale={language === "th" ? thLocale : enUS}
                modifiers={{
              ...
              <div className="w-full flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                <span className="inline-flex items-center gap-1.5">
                  ...
                  {tx("วันที่มอบหมาย", "Assigned date")}
                </span>
                <span className="inline-flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full bg-destructive" />
                  {tx("กำหนดส่ง", "Deadline")}
                </span>
              </div>
            </CardContent>
          </Card>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold">{formatDateHeader(selectedDate, language)}</h2>
            <Badge variant="secondary" className="rounded-full">
              {isTeacher ? assignmentsDueOnSelectedDate.length : tasksOnSelectedDate.length} {tx("รายการ", "item(s)")}
            </Badge>
          </div>
            ...
            <Card className="border-none shadow-sm rounded-2xl">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-semibold flex items-center gap-2">
                  ...
                  {isTeacher ? tx("งานที่ประกาศในวันนี้", "Assignments posted on this day") : tx("งานที่ได้รับมอบหมายในวันนี้", "Work assigned on this day")}
                </CardTitle>
              </CardHeader>
                ...
                {assignmentsAssignedOnSelectedDate.length > 0 ? (
                  assignmentsAssignedOnSelectedDate.map((assignment) => (
                    <Link key={`assigned-${assignment.id}`} to={`/task/${assignment.id}`}>
                      <motion.div whileHover={{ y: -2 }} className="rounded-xl border border-border p-3 hover:bg-accent/40 transition-colors">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <Badge variant="outline" className="text-[11px]">
                            {formatTime(assignment.createdAt, language)}
                          </Badge>
                        </div>
                      </motion.div>
                    </Link>
                  ...
                  <p className="text-sm text-muted-foreground">{tx("ไม่มีงานประกาศในวันที่เลือก", "No assignments were posted on this date.")}</p>
                )}
              </CardContent>
            </Card>
          </motion.div>
            ...
            <Card className="border-none shadow-sm rounded-2xl">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-semibold flex items-center gap-2">
                  ...
                  {isTeacher ? tx("กำหนดส่งในวันนี้", "Deadlines on this day") : tx("กำหนดส่งของฉันในวันนี้", "My deadlines on this day")}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-2">
                {isTeacher ? (
                  assignmentsDueOnSelectedDate.length > 0 ? (
                      ...
                      <Link key={`deadline-${assignment.id}`} to={`/task/${assignment.id}`}>
                        <motion.div whileHover={{ y: -2 }} className="rounded-xl border border-border p-3 hover:bg-accent/40 transition-colors">
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <Badge variant="outline" className="text-[11px]">
                              {formatTime(assignment.submission.deadline, language)}
                            </Badge>
                          </div>
                        </motion.div>
                      </Link>
                    ...
                    <p className="text-sm text-muted-foreground">{tx("ไม่มีงานที่ถึงกำหนดส่งในวันที่เลือก", "No deadlines on this date.")}</p>
                  )
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            ...
                            <div className="text-xs text-muted-foreground mt-1 flex items-center gap-2">
                              ...
                              <span>{getTaskStatusLabel(task.status, language)}</span>
                            </div>
                          </div>
                          <Badge variant="outline" className="text-[11px] inline-flex items-center gap-1">
                            <Clock className="w-3 h-3" />
                            {formatTime(task.deadline, language)}
                          </Badge>
                        </div>
                      </motion.div>
                    </Link>
                  ...
                  <div className="py-3 text-sm text-muted-foreground inline-flex items-center gap-2">
                    ...
                    {tx("ไม่มีงานที่กำหนดส่งในวันที่เลือก", "No deadline items on this date.")}
                  </div>
                )}
              </CardContent>
            </Card>
